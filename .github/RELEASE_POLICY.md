...
Release Please runs only after both CI and CodeQL have completed successfully for the same main commit. It maintains one release pull request from conventional commits. Merging that pull request updates package.json and CHANGELOG.md, creates the matching vX.Y.Z tag, and publishes the GitHub Release. The release workflow then builds and smoke-tests that exact commit before publishing vX.Y.Z and latest container tags. It does not publish an npm package.
